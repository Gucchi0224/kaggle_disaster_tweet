...
if __name__ == "__main__":
    ...
    tfidf, sentences = calc_tfidf(raw_tokens)
    #X = np.array(tfidf)
    ...
    # モデル構築
    inputs = Input(shape=(MAX_SEQUENCE,), dtype='int32')
    x = wv_model.get_keras_embedding(train_embeddings=True)(inputs)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for (X_batch_train, y_batch_train, X_batch_val, y_batch_val) in zip(X_split_train, y_split_train, X_split_val, y_split_val):
            history = model.fit(X_batch_train, y_batch_train, validation_data=(X_batch_val, y_batch_val), callbacks=callback)
    ...
    plt.savefig("loss-acc\\BiLSTM-word2vec-batch.jpg", bbox_inches='tight')
    plt.close()
    ...
    plt.savefig("result_val\\BiLSTM-word2vec-batch.jpg", bbox_inches='tight')
    plt.close()
    ...
    pred_df.to_csv("predict_csv\\prediction_data_by_BiLSTM_word2vec_batch.csv", index_label=["id"])
